Talker.ReceivedSound = function() {
  ...
  plugin.onCommand = function(talkerEvent) {
    if (talkerEvent.command == 'togglesound') {
  ...
  plugin.onLoaded = function() {
    ...
    $(document.body).append($('<audio/>').attr('src', '/javascripts/borealis/message_received.wav')); // preloads for faster play on first message.
      
    plugin.onMessageReceived = function(talkerEvent) {
      if (talkerEvent.user.id != Talker.currentUser.id && $.cookie('ReceivedSound') == 'true') {
        $(document.body).append(
          $('<audio/>').attr('src', '/javascripts/borealis/message_received.wav').attr('autoplay', 'true').bind('ended', function(){ $(this).remove() })
        );
      }
    }
  }
